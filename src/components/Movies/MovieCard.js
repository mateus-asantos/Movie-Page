    ...
    return (
        <section className="Filme-card">
            ...
            <div className="Filme-info">
                ...
                <p className="Filme-sinopse">{overview}{endstring ? <strong>Leia Mais</strong> : ""}</p>
                <ul className="Filme-lista-generos">
                    {props.movie.genre_ids.map(genre => (
                        <li className="Filme-genero" key={genre.name}>{props.genres.find(item => item.id === genre).name}</li>
                    ))}
                </ul>
            </div>
